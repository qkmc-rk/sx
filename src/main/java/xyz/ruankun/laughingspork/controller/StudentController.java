...
public class StudentController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "stage1GuideDate", value = "指导时间", required = true),
    })
    @RequiresRoles("Student")
    @PostMapping("/report/stage1")
    public ResponseVO stage1Summary(String stage1Summary, String stage1GuideWay, String stage1GuideDate) {
        if (stage1Summary == null) {
            return ControllerUtil.getFalseResultMsgBySelf(RespCode.MSG_INCOMPLETE_DATA);
        } else {
            //保存鉴定表内容到数据库
            SxReport sxReport = sxStudentService.stage1_summary(
                    (SxStudent) SecurityUtils.getSubject().getPrincipal(),
                    stage1Summary, stage1GuideWay, stage1GuideDate);
            if (sxReport != null) {
                return ControllerUtil.getDataResult(sxReport);
            } else {
                return ControllerUtil.getFalseResultMsgBySelf(RespCode.MSG_NOT_FOUND_DATA);
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "stage2GuideDate", value = "指导时间", required = true),
    })
    @RequiresRoles("Student")
    @PostMapping("/report/stage2")
    public ResponseVO stage2Summary(String stage2Summary, String stage2GuideWay, String stage2GuideDate) {
        if (stage2Summary == null) {
            return ControllerUtil.getFalseResultMsgBySelf(RespCode.MSG_INCOMPLETE_DATA);
        } else {
            //保存鉴定表内容到数据库
            SxReport sxReport = sxStudentService.stage2_summary(
                    (SxStudent) SecurityUtils.getSubject().getPrincipal(),
                    stage2Summary, stage2GuideWay, stage2GuideDate);
            if (sxReport != null) {
                return ControllerUtil.getDataResult(sxReport);
            } else {
                return ControllerUtil.getFalseResultMsgBySelf(RespCode.MSG_NOT_FOUND_DATA);
